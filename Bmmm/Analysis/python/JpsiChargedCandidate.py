...
# The PV-refit machinery (constants, track filter, vertex builders and
# refit_primary_vertex itself) lives in PrimaryVertex.py so the dimuon channel
# runs the SAME code rather than a second copy that drifts.
...
##########################################################################################
# The PV refit is shared with the dimuon channel (Bmmm.Analysis.PrimaryVertex).
# It is attached here rather than put in the class bases because
# JpsiChargedCandidate derives from a cppyy class (ROOT.reco.CompositeCandidate),
# and adding a python base to that is a change to how the class itself is built,
# which this refactor has no reason to risk. MuMuCandidate is plain python and
# simply inherits the mixin.
##########################################################################################
for _shared in ('refit_primary_vertex', 'is_signal_muon_cand'):
    setattr(JpsiChargedCandidate, _shared, getattr(PrimaryVertexRefitMixin, _shared))
