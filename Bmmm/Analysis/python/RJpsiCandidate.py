...
class RJpsiCandidate(ROOT.reco.CompositeCandidate):
    ...
    def compute_vtx_quantities(self, vertices, beamspot):
        ...
        self.compute_displacement(self.jpsi_vertex_tree, self.jpsi_good_vtx, self.jpsi, prefix='jpsi_')
    ...
    def compute_ip(self):
        ...
        for q in ['mu_ip3d', 'mu_ip3d_err', 'mu_ip3d_sig',
                  'mu_jpsi_ip3d', 'mu_jpsi_ip3d_err', 'mu_jpsi_ip3d_sig']:
            setattr(self, q, np.nan)

        # the flight direction needs a good 3-muon secondary vertex
        if not self.good_vtx:
            return

        # Bc flight direction: PV -> 3-muon SV
        sv = self.vtx.position()
        dx = sv.x() - self.pv.position().x()
        dy = sv.y() - self.pv.position().y()
        dz = sv.z() - self.pv.position().z()

        track = self.mu.bestTrack()

        # ----- w.r.t. the primary vertex ------------------------------------
        ip = kinfit.signedIP3D(track, dx, dy, dz, self.pv)
        self.mu_ip3d     = ip.value()
        self.mu_ip3d_err = ip.error()
        self.mu_ip3d_sig = ip.significance()

        # ----- w.r.t. the J/psi vertex --------------------------------------
        if self.jpsi_good_vtx:
            jpsi_vertex = self.kin_to_reco_vertex(self.jpsi_vtx)
            ip = kinfit.signedIP3D(track, dx, dy, dz, jpsi_vertex)
            self.mu_jpsi_ip3d     = ip.value()
            self.mu_jpsi_ip3d_err = ip.error()
            self.mu_jpsi_ip3d_sig = ip.significance()
