...
What it actually checks -- in order of how badly a silent failure would hurt:
  ...
  8. an event-level context (npv) without an event is a hard error too, rather
     than a correction conditioned on a stale or absent pileup value.
'''
...
def main():
    ...
    try:
        ...
        print('\n== 8. an event-level context without an event is refused ==')
        tmp2 = tempfile.mkdtemp(prefix='covflow_test_npv_')
        try:
            make_run_directory(tmp2, ['log_pt', 'eta', 'n_pix_hit', 'npv'])
            corr_npv = make_cov_corrector(tmp2)
            check('needs_event is set from the context list', corr_npv.needs_event)
            mu_npv = FakeMuon(fake_track(random_cov(seed=11)))
            try:
                corr_npv.prime([mu_npv])
            except RuntimeError as exc:
                check('prime() without an event raises', 'npv' in str(exc))
            else:
                check('prime() without an event raises', False)
            check('a track-only context does not need an event',
                  not corr.needs_event)
        finally:
