...
python3 derive_sigma_correction_v2.py --var abs_mu2_bs_dxy_e \
    --name mu2_bs_dxy_e_morph --nquantiles 20 --out sigma_dxy_morph_v2.json
...
def build_map_smooth(edges, counts_mc, counts_data, nq=50, kde_bw=0.0):
    ...
    pch = PchipInterpolator(xq_mc, xq_da, extrapolate=True)
    lo, hi = edges[0], edges[-1]

    def T(x):
        x = np.clip(np.asarray(x, float), lo, hi)
        return np.clip(pch(x), lo, hi)
...
# ----------------------------------------------------------------------------
def _sigma_edges(edges, nsample):
    return np.linspace(edges[0], edges[-1], nsample + 1)


def sigma_binning(edges, Tfunc, nsample):
    xs = _sigma_edges(edges, nsample)
...
def content_binning(edges, values, nsample):
    xs = _sigma_edges(edges, nsample)
    return {"nodetype": "binning", "input": "sigma_dxy",
            "edges": [float(e) for e in xs], "flow": "clamp",
...
def build_correctionset(cells, name, nq, kde_bw, nsample, min_events,
    ...
    xs = _sigma_edges(edges0, nsample)
    xs_c = 0.5 * (xs[:-1] + xs[1:])
    ...
    for cell in all_cells:
        ...
        nominal[cell] = sigma_binning(edges, T, nsample)
        ...
        if nboot > 0 and ok:
            ...
            up[cell] = content_binning(edges,
                                       np.maximum.accumulate(nom + sd), nsample)
...
def main():
    ...
    ap.add_argument("--var", default="abs_mu2_dxy_e",
                    help="set to abs_mu2_bs_dxy_e for the beam-spot variable")
