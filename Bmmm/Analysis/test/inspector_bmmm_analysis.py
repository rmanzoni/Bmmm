...
from Bmmm.Analysis.branches import branches, paths
from Bmmm.Analysis.Candidate import Candidate
...
for i, event in enumerate(events):
    ...
    for itriplet in combinations(muons, 3): 

        # 3 muon candidate
        cand = Candidate(itriplet, event.vtx, event.bs)
        
        # 3 muons somewhat close in dz, max distance 1 cm
        ...
        if cand.mass_corrected()>7. or cand.mass_corrected()<4.:
            continue
    ...
    tofill['min_mass'] = min([final_cand.mass12(), final_cand.mass13(), final_cand.mass23()])
