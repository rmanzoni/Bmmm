...
## tighten muon ID
cuts['4mu']['mu_id'] = lambda mu : mu.isMediumMuon() and mu.isPFMuon() and mu.isGlobalMuon()
...
async def looper(events, options, handles, handles_mc, row_list, start):
    ...
    async for i, event in aiostream.stream.enumerate(AsyncIter(events), 1):
        ...
        triggers_tofill = dict()
        for ipath in paths.keys():
            triggers_tofill[ipath        ] = np.nan
            triggers_tofill[ipath + '_ps'] = np.nan
        ...
        for iname in trg_names.triggerNames():
            ...
            for ipath in paths.keys():
                ...
                if drop_hlt_version(iname)==ipath:
                    ...
                    triggers_tofill[ipath        ] = ( idx < len(trg_names)) * (event.trg_res.accept(idx))
                    triggers_tofill[ipath + '_ps'] = event.trg_ps.getPrescaleForIndex(idx)
        
        triggers = {key:triggers_tofill[key] for key in paths.keys()}
        ...
        ######################################################################################
        #####      FILL ONE ENTRY PER CANDIDATE
        ######################################################################################

        for final_cand in cands:
            
            final_cand.compute_dimuon_vtx_quantities()

            # reset trees
            tofill = dict(zip(branches, [np.nan]*len(branches)))

            # save trigger info
            for k, v in triggers_tofill.items():
                tofill[k] = v
